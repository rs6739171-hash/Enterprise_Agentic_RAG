...
| P2 | `app/config.py` and gateway | Configuration validation was unused and did not require Portkey; gateway retries/cache/fallback config was never attached. Validate actual dependencies, lazily initialize clients, and attach the gateway config. |
